...
drop table if exists 游客;

drop table if exists 管理员;

drop table if exists 视频课程;
...
alter table 习题 add constraint FK_做题 foreign key (用户id)
      references 游客 (用户id) on delete restrict on update restrict;
...
alter table 习题 add constraint FK_课程习题 foreign key (课程id)
      references 视频课程 (课程id) on delete restrict on update restrict;

alter table 已登录用户 add constraint FK_登录 foreign key (用户id)
      references 游客 (用户id) on delete restrict on update restrict;

alter table 视频课程 add constraint FK_用户发布视频 foreign key (已登录_用户id)
      references 已登录用户 (用户id) on delete restrict on update restrict;

alter table 视频课程 add constraint "FK_管理员发布/审核视频" foreign key (管理员id)
      references 管理员 (管理员id) on delete restrict on update restrict;

alter table 视频课程 add constraint FK_观看视频课程 foreign key (用户id)
      references 游客 (用户id) on delete restrict on update restrict;
